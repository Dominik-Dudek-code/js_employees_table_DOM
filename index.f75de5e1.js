...
function formatString(input) {
    ...
    if (items.length === 1) return `${items[0]} is`;
...
form.addEventListener("submit", (e)=>{
    ...
    let whichEmptyField = "";
    for (const [key, value1] of formData)if (!value1.trim()) {
        isEmptyField = true;
        whichEmptyField += `${key} `;
    }
    if (isEmptyField) {
        const emptyFields = formatString(whichEmptyField.trim());
        createNotification("error", `Failed because ${emptyFields} empty, please enter correct value to inputs!`);
        return;
    }
    for (const [key, value1] of formData){
        currentError = validate(key, value1);
        if (currentError) return;
    }
    createNotification("success", "New employee has been added to the table!");
    // creating employee object for adding row
    const employeeData = {};
    for(const field in columnIndexMap)employeeData[field] = formData.get(field);
...
function validate(key, value1) {
    if (key === "name" || key === "position") {
        if (value1.length < 4) {
            createNotification("error", `${createFirstBigLetter(key)} should be at least 4 letters long!`);
            return true;
        }
        if (value1.length > 40) {
            createNotification("error", `${createFirstBigLetter(key)} can't be that long!`);
            return true;
        }
        if (!/^[a-zA-Z]+(?: [a-zA-Z]+)*$/.test(key)) {
            createNotification("error", `${createFirstBigLetter(key)} should contain letters, with only one space between words!`);
            return true;
        }
    }
    if (key === "age") {
...
function makeSalary(value1) {
    return `$${Number(value1).toLocaleString("en-US")}`;
}
...
rows.addEventListener("dblclick", (e)=>{
    ...
    let headerCell;
    for(const key in columnIndexMap)if (key === "id") headerCell = columnIndexMap[key];
    ...
    for(const key in columnIndexMap)if (columnIndexMap[key] === cellName) {
        cellName = key;
        break;
    }
...
function makeEditInput(inputName, type) {
    ...
    const formCopy = document.querySelector(`${typeCreated}[name="${inputName}"]`);
